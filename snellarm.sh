...
cd ~/
wget --no-check-certificate -O snell.zip https://github.com/surge-networks/snell/releases/download/v2.0.4/snell-server-v2.0.4-linux-armv7l.zip
unzip -o snell.zip
rm -f snell.zip
...
if [ -f ${CONF} ]; then
  ...
  echo "listen = 0.0.0.0:16358" >>${CONF}
  echo "psk = ${PSK}" >>${CONF}
  echo "obfs = tls" >>${CONF}
fi
if [ -f ${SYSTEMD} ]; then
